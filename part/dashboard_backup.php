...
$queryRooms = "SELECT DISTINCT room FROM records_log GROUP BY room ASC";
...
$roomCounts = [];
$totalCountRooms = 0;
$queryRoomCounts = "SELECT room, COUNT(*) as count FROM records_log GROUP BY room ASC";
...
$roomquery = "SELECT DISTINCT room FROM records_issue GROUP BY room ASC";
...
$selectedissueroom = isset($_GET['issue_room']) ? $conn->real_escape_string($_GET['issue_room']) : '';  // Corrected the room selection handling

$roomcount = [];
$totalissuecountrooms = 0;
$totalqueryroomcounts = "SELECT room, COUNT(*) as count1 FROM records_issue GROUP BY room ASC";
...
if ($selectedissueroom) {
    // Prepared statement for issues on selected room
...
?>
    ...
    <button class="button no-print" onclick="printPage()">Print This Page</button>
            ...
            <div style="display: flex; flex-wrap: wrap; height: auto; margin-top: 1%; margin-left: 1%; margin-right: 1%;">
